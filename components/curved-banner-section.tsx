...
export default function CurvedBannerSection({
    ...
    const fetchBanner = async () => {
      try {
        ...
        const response = await fetch(
          `${baseUrl}/banners?position=${position}&pageSlug=${
            pageSlug || "home"
          }&isActive=true`
        );
        ...
        if (data.success && data.data.length > 0) {
          ...
          const getStr = (v: any) =>
            typeof v === "string" ? v : v?.ar || v?.en || "";

          const mapped: CurvedBannerSection = {
            _id: bannerData._id,
            content:
              getStr(bannerData.title) || getStr(bannerData.content) || "—",
            ...
            ctaButton: bannerData.ctaButton
              ? {
                  text:
                    getStr(bannerData.ctaButton.text) ||
                    bannerData.ctaButton.text,
            ...
            curveDepth:
              bannerData.curveIntensity === "light"
                ? 40
                : bannerData.curveIntensity === "strong"
                ? 120
                : 80,
  ...
  return (
    <div
      ...
      style={{ marginTop: containerOffset }}
    >
            ...
            {banner.ctaButton && (
              <div className="pt-6">
                <a
                  ...
                  className={`inline-flex items-center gap-4 px-10 md:px-12 py-5 md:py-6 rounded-lg text-xl md:text-2xl transition-all duration-300 transform hover:scale-105 hover:shadow-2xl ${getButtonStyle(
                    banner.ctaButton.style
                  )} arabic-text `}
