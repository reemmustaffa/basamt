...
export function Footer() {
    ...
    const load = async () => {
      try {
        ...
        const [resFooter, resSocial, resContact] = await Promise.all([
          ...
          }>("/settings?category=footer", { method: "GET" }),
          apiFetch<{
            success: boolean;
            data: Array<{
              key: string;
          ...
          }>("/settings?category=social", { method: "GET" }),
          apiFetch<{
            success: boolean;
            data: Array<{
              key: string;
          ...
          }>("/settings?category=contact", { method: "GET" }),
        ]);

        const dataFooter =
          (resFooter as any)?.success && Array.isArray((resFooter as any).data)
            ? (resFooter as any).data
            : [];
        const dataSocial =
          (resSocial as any)?.success && Array.isArray((resSocial as any).data)
            ? (resSocial as any).data
            : [];
        const dataContact =
          (resContact as any)?.success &&
          Array.isArray((resContact as any).data)
            ? (resContact as any).data
            : [];
        ...
        const contactRes = (await apiFetch("/contact-page", {
          method: "GET",
        })) as any;
        if (contactRes?.success && contactRes.data) {
          ...
          const socialLinkedIn =
            d.socialLinkedin || d.socialMediaSection?.platforms?.linkedin || "";
          const socialInstagram =
            d.socialInstagram ||
            d.socialMediaSection?.platforms?.instagram ||
            "";
          const socialTwitter =
            d.socialTwitter || d.socialMediaSection?.platforms?.twitter || "";
          const socialTiktok =
            d.socialTiktok || d.socialMediaSection?.platforms?.tiktok || "";
  ...
  const brandText =
    normalizeText(cfg?.brandText) || "نصمم، نكتب، ونبني لك هوية تترك أثرًا";
  const email = normalizeText(cfg?.email) || "contact@basmadesign.com";
  const instagram = normalizeLink(
    cfg?.instagram ??
      (cfg as any)?.instagramUrl ??
      (cfg as any)?.instagramURL ??
      (cfg as any)?.instagram_link
  );
  const linkedin = normalizeLink(
    cfg?.linkedin ??
      (cfg as any)?.linkedinUrl ??
      (cfg as any)?.linkedinURL ??
      (cfg as any)?.linkedin_link
  );
  const facebook = normalizeLink(
    (cfg as any)?.facebook ??
      (cfg as any)?.facebookUrl ??
      (cfg as any)?.facebookURL ??
      (cfg as any)?.facebook_link
  );
  const x = normalizeLink(
    (cfg as any)?.x ??
      (cfg as any)?.twitter ??
      (cfg as any)?.twitterUrl ??
      (cfg as any)?.twitterURL ??
      (cfg as any)?.twitter_link
  );
  const tiktok = normalizeLink(
    (cfg as any)?.tiktok ??
      (cfg as any)?.tiktokUrl ??
      (cfg as any)?.tiktokURL ??
      (cfg as any)?.tiktok_link
  );
  const youtube = normalizeLink(
    (cfg as any)?.youtube ??
      (cfg as any)?.youtubeUrl ??
      (cfg as any)?.youtubeURL ??
      (cfg as any)?.youtube_link
  );
  const behance = normalizeLink(
    (cfg as any)?.behance ??
      (cfg as any)?.behanceUrl ??
      (cfg as any)?.behanceURL ??
      (cfg as any)?.behance_link
  );
  const dribbble = normalizeLink(
    (cfg as any)?.dribbble ??
      (cfg as any)?.dribbbleUrl ??
      (cfg as any)?.dribbbleURL ??
      (cfg as any)?.dribbble_link
  );
  const snapchat = normalizeLink(
    (cfg as any)?.snapchat ??
      (cfg as any)?.snapchatUrl ??
      (cfg as any)?.snapchatURL ??
      (cfg as any)?.snapchat_link
  );
  const whatsapp = normalizeLink(
    (cfg as any)?.whatsapp ??
      (cfg as any)?.whatsApp ??
      (cfg as any)?.whatsappUrl ??
      (cfg as any)?.whatsAppUrl ??
      (cfg as any)?.whatsappURL ??
      (cfg as any)?.whatsapp_link
  );
  ...
  const isValidLink = (url: string) => {
    ...
    if (lower.includes("your-number") || lower.includes("yournumber"))
      return false;
    return true;
  };
  ...
  const finalWhatsAppLink = isValidWhats(whatsAppFromContact)
    ? whatsAppFromContact
    : "";
  ...
  const quickLinks: Array<any> = Array.isArray(cfg?.quickLinks)
    ? (cfg!.quickLinks as any[])
    : defaultQuickLinks;
  ...
  if (!hasHowTo) {
    normalizedQuickLinks.splice(2, 0, {
      label: "كيف تطلب خدمتك",
      href: "/how-to-order",
    });
  }
  const servicesLinks: Array<{ label: string; href: string }> = Array.isArray(
    cfg?.servicesLinks
  )
    ? cfg!.servicesLinks
    : [
  ...
  return (
    <div className="relative">
      ...
      <footer
        className="relative overflow-visible mt-24 sm:mt-8 md:mt-24 lg:mt-32 xl:mt-40 2xl:mt-48"
        ...
        <div className="container mx-auto px-4 py-12 sm:py-16 md:py-20 relative z-20 mt-8 sm:mt-12 md:mt-16">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 auto-rows-max items-start">
            ...
            <div className="space-y-4 sm:space-y-6 text-center sm:text-right sm:col-span-2 lg:col-span-1 relative z-20">
              <div className="flex items-center justify-center sm:justify-start">
                <Image
                  ...
                  style={{
                    filter:
                      "invert(1) grayscale(1) contrast(1000%) brightness(1000%)",
                  }}
                />
              </div>
              <p className="text-white/80 arabic-text leading-relaxed text-sm sm:text-base">
                {brandText}
              </p>
              <div className="flex gap-2 sm:gap-3 justify-center sm:justify-start relative z-20">
                ...
                <a
                  ...
                  rel={
                    mainContactIsWhatsApp ? "noopener noreferrer" : undefined
                  }
                >
                ...
                {email && (
                  <a
                    href={`mailto:${email}`}
                    className="group relative inline-flex items-center justify-center p-0.5 rounded-full bg-gradient-to-r from-white/40 to-white/20 hover:from-white/70 hover:to-white/30 transition-all duration-300 hover:scale-110"
                ...
                {isValidLink(instagram) && (
                  <a
                    ...
                    className="group relative inline-flex items-center justify-center p-0.5 rounded-full bg-gradient-to-r from-pink-400/70 to-fuchsia-400/70 hover:from-pink-300 hover:to-fuchsia-300 transition-all duration-300 hover:scale-110"
                ...
                {isValidLink(linkedin) && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group relative inline-flex items-center justify-center p-0.5 rounded-full bg-gradient-to-r from-sky-400/70 to-blue-500/70 hover:from-sky-300 hover:to-blue-400 transition-all duration-300 hover:scale-110"
                    aria-label="LinkedIn"
                  >
                    <span className="inline-flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full bg-white/10 text-white backdrop-blur-sm">
                ...
                {isValidLink(facebook) && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group relative inline-flex items-center justify-center p-0.5 rounded-full bg-gradient-to-r from-blue-500/70 to-blue-600/70 hover:from-blue-400 hover:to-blue-500 transition-all duration-300 hover:scale-110"
                    aria-label="Facebook"
                  >
                    <span className="inline-flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full bg-white/10 text-white backdrop-blur-sm">
                ...
                {isValidLink(x) && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group relative inline-flex items-center justify-center p-0.5 rounded-full bg-gradient-to-r from-white/60 to-white/30 hover:from-white hover:to-white/70 transition-all duration-300 hover:scale-110"
                ...
                {isValidLink(tiktok) && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group relative inline-flex items-center justify-center p-0.5 rounded-full bg-gradient-to-r from-teal-400/70 to-pink-500/70 hover:from-teal-300 hover:to-pink-400 transition-all duration-300 hover:scale-110"
                    aria-label="TikTok"
                  >
                    <span className="inline-flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full bg-white/10 text-white backdrop-blur-sm">
                ...
                {isValidLink(youtube) && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group relative inline-flex items-center justify-center p-0.5 rounded-full bg-gradient-to-r from-red-500/80 to-rose-500/80 hover:from-red-400 hover:to-rose-400 transition-all duration-300 hover:scale-110"
                    aria-label="YouTube"
                  >
                    <span className="inline-flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full bg-white/10 text-white backdrop-blur-sm">
                ...
                {isValidLink(behance) && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group relative inline-flex items-center justify-center p-0.5 rounded-full bg-gradient-to-r from-blue-500/70 to-indigo-500/70 hover:from-blue-400 hover:to-indigo-400 transition-all duration-300 hover:scale-110"
                    aria-label="Behance"
                  >
                    <span className="inline-flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full bg-white/10 text-white backdrop-blur-sm">
                ...
                {isValidLink(dribbble) && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group relative inline-flex items-center justify-center p-0.5 rounded-full bg-gradient-to-r from-pink-400/80 to-rose-400/80 hover:from-pink-300 hover:to-rose-300 transition-all duration-300 hover:scale-110"
                    aria-label="Dribbble"
                  >
                    <span className="inline-flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full bg-white/10 text-white backdrop-blur-sm">
                ...
                {isValidLink(snapchat) && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group relative inline-flex items-center justify-center p-0.5 rounded-full bg-gradient-to-r from-yellow-300/80 to-amber-300/80 hover:from-yellow-200 hover:to-amber-200 transition-all duration-300 hover:scale-110"
                    aria-label="Snapchat"
                  >
            ...
            <div className="space-y-3 sm:space-y-4 text-center sm:text-right relative z-20">
              <h3 className="font-semibold text-white arabic-text text-base sm:text-lg">
                روابط سريعة
              </h3>
              <nav className="flex flex-col gap-2 sm:gap-3">
                {normalizedQuickLinks.map((l, i) => (
                  <Link
                    key={i}
                    href={l.href}
                    className="footer-link !text-white arabic-text group flex items-center justify-center sm:justify-start gap-2 text-sm sm:text-base"
                    style={{
                      textDecoration: "none !important",
                    }}
                  >
                    <ArrowLeft
                      className="h-3 w-3 !text-white rtl-flip opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all "
                      style={{
                        color: "#fff !important",
                        fill: "#fff !important",
                        stroke: "#fff !important",
            ...
            <div className="space-y-3 sm:space-y-4 text-center sm:text-right sm:col-span-2 lg:col-span-1 relative z-20">
              ...
              <div className="space-y-3">
                {email && (
                  <a
                    href={`mailto:${email}`}
                    className="footer-link !text-white  block text-sm sm:text-base break-all"
                    style={{
                      color: "#fff !important",
                      textDecoration: "none !important",
                    }}
                ...
                <div className="flex items-center justify-center sm:justify-start gap-2 flex-wrap">
                  <div className="flex items-center gap-2 bg-gradient-to-r from-white/20 to-white/10 hover:from-white/30 hover:to-white/20 px-2 sm:px-3 py-1 sm:py-2 rounded-full border border-blue-200">
                    <svg
                      width="16"
                      height="16"
                    ...
                    >
                      ...
                      />
                      <path
                        d="M6.232 21.337H1.626a.641.641 0 0 1-.633-.74L4.1.901C4.182.382 4.63 0 5.154 0h7.46c2.57 0 4.578.543 5.69 1.81 1.01 1.15 1.304 2.42 1.012 4.287-.023.143-.047.288-.077.437-.983 5.05-4.349 6.797-8.647 6.797H8.402c-.524 0-.968.382-1.05.9l-1.12 7.106z"
                        fill="#0070BA"
                      />
                    </svg>
                    <span
                  ...
                  <span className="text-xs text-white/60 arabic-text">
                    دفع آمن
                  </span>
                </div>
                <Button
                  size="sm"
                  className="bg-gradient-to-r from-white/20 to-white/10 hover:from-white/30 hover:to-white/20 font-medium arabic-text group transition-all duration-300 hover:scale-105 mt-3 sm:mt-4 border border-white/30 text-sm sm:text-base w-full sm:w-auto "
                  asChild
                >
                  <Link
                    href={finalWhatsAppLink || contactCtaLink}
                  ...
                  >
                    ...
                    <ArrowLeft
                      className="h-3 !text-white w-3 sm:h-4 sm:w-4 rtl-flip group-hover:translate-x-1 transition-transform"
                      style={{
                        color: "#fff !important",
                        fill: "#fff !important",
                        stroke: "#fff !important",
          ...
          <div className="border-t border-white/20 mt-8 sm:mt-10 md:mt-12 pt-6 sm:pt-8 text-center">
            <p className="text-white/80 arabic-text text-sm sm:text-base">
              {copyright}
            </p>
            <p className="text-xs sm:text-sm text-white/60 mt-2 arabic-text opacity-75">
              {refundNote}
            </p>
          </div>
        </div>
