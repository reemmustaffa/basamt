...
export function ServiceFeaturesTimeline({ features }: ServiceFeaturesTimelineProps) {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl sm:text-2xl font-bold text-white arabic-text mb-6 bg-[#4B2E83] px-4 sm:px-6 py-3 rounded-lg inline-block">
        مميزات الخدمة
      </h2>
      
      {/* Desktop Timeline - مخفي على الموبايل */}
      ...
      {/* Mobile & Tablet Grid - محسن للموبايل */}
      <div className="lg:hidden">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          {features.map((feature: string, index: number) => (
            <div key={`mobile-${index}`} className="flex items-start gap-3 p-3 sm:p-4 bg-gradient-to-r from-[#7a4db3] to-[#4b2e83] text-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-300">
              <div className="flex-shrink-0 mt-0.5">
                <CheckCircle className="h-4 w-4 sm:h-5 sm:w-5 text-white/90" />
              </div>
              <span className="arabic-text text-sm sm:text-base font-medium leading-relaxed text-right">
                {feature}
              </span>
            </div>
          ))}
