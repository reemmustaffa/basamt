...
export function HeroSection() {
  ...
  useEffect(() => {
    const loadHero = async () => {
      try {
        const res = await apiFetch<HeroData>("/hero-section", {
          method: "GET",
          cache: "no-store",
        });
        setHero(res);
  ...
  const description =
    hero?.description?.ar ||
    "في بصمة تصميم، نحول أفكارك إلى تصاميم استثنائية تعكس شخصيتك وتحقق أهدافك";
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="order-2 lg:order-1 text-center lg:text-right space-y-8 animate-fade-in">
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold  arabic-text text-primary text-balance leading-tight">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-end">
                <Button
                  size="lg"
                  className="bg-gradient-to-r from-primary to-accent hover:from-accent hover:to-primary font-semibold text-lg px-8 py-4 arabic-text group transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl hover:shadow-primary/25 !text-white"
                  asChild
                >
                  <Link
                    href="/services"
                  ...
                  >
                    <span style={{ color: "#fff !important" }}>اطلب خدمة</span>
                    <ArrowLeft
                      className="h-5 w-5 rtl-flip group-hover:translate-x-1 transition-transform"
                      style={{ color: "#fff !important" }}
                    />
                  </Link>
                </Button>
                <Button
                  size="lg"
                  variant="outline"
                  className="border-2 border-primary/30 hover:bg-primary font-semibold text-lg px-8 py-4 arabic-text group transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl hover:shadow-primary/25 "
                  asChild
                >
                  <Link
                    href={ctaLink}
                    className="flex items-center gap-2 text-[#7a4db3]"
                  >
                    <span className="text-[#7a4db3]">{ctaText}</span>
                    <ArrowLeft className="h-5 w-5 rtl-flip text-[#7a4db3] group-hover:translate-x-1 transition-transform" />
                  </Link>
                </Button>
              </div>
            </div>
            ...
            <div className="order-1 lg:order-2 flex justify-center animate-slide-up">
              <div className="relative">
                ...
                <div className="absolute -top-6 -right-6 bg-card/90 backdrop-blur-sm border border-border rounded-xl p-2 text-center hover:bg-card transition-all duration-300 hover:scale-110 shadow-lg animate-bounce">
                  ...
                  <p className="text-foreground text-xs font-semibold arabic-text">
                    جودة عالية
                  </p>
                </div>
