...
export function FoundationalStatement() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const res = await apiFetch<any>("/foundational", {
          method: "GET",
          cache: "no-store",
        });
  ...
  const title = pickText(cfg?.title) || (
    <div className="text-[#7a4db3]">
      في بصمة تصميم، نمنح مشروعك <br />
      <span className="text-transparent bg-gradient-to-r from-primary text-[#7a4db3] to-accent bg-clip-text">
        حضورًا لا يُنسى
      </span>
    </div>
  );
  const subtitle =
    pickText(cfg?.subtitle) ||
    "نصمم، نكتب، ونبني لك هوية تترك أثرًا في قلوب عملائك";
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            ...
            <Button
              variant="outline"
              size="lg"
              className="font-semibold arabic-text border-2 border-primary/20 hover:bg-primary/10 hover:border-primary/40 bg-card/50 backdrop-blur-sm text-lg px-8 py-4 "
              asChild
            >
              <Link href={ctaSecondaryLink} className="text-[#7a4db3]">
                <span className="text-[#7a4db3]">{ctaSecondaryText}</span>
              </Link>
            </Button>
          </div>
        </div>
