...
export default async function PoliciesPage() {
  ...
  // Load contact email
  try {
    const contactResponse: any = await apiFetch("/settings?category=contact", {
      method: "GET",
    });
        ...
        <div className="container mx-auto px-4 text-center relative">
          <div className="max-w-4xl mx-auto">
            ...
            <p className="text-xl md:text-2xl text-muted-foreground arabic-text mb-8 leading-relaxed">
              معلومات مهمة وشاملة حول شروط الخدمة وسياساتنا لضمان تجربة واضحة
              ومفهومة
            </p>
          </div>
        </div>
          ...
          <Card className="mb-12 bg-gradient-to-r from-amber-50 to-orange-50 border-amber-200/70 overflow-hidden relative">
            ...
            <CardContent className="p-8 relative">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-bold text-amber-800 mb-2 arabic-text text-lg">
                    تنويه مهم
                  </h3>
                  <p className="text-amber-700 arabic-text leading-relaxed">
                    يرجى قراءة جميع السياسات بعناية قبل طلب أي خدمة. استخدام
                    خدماتنا يعني موافقتك على جميع الشروط والأحكام المذكورة
                    أدناه.
                  </p>
                </div>
              </div>
            ...
            {policies.map((policy, index) => {
              ...
              return (
                <Card
                  key={policy.id}
                  id={policy.id}
                  className={`group hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-violet-200/70 overflow-hidden relative ${
                    hasContent ? "h-auto" : "h-fit"
                  }`}
                >
                  <div
                  ...
                  <CardHeader className="relative pb-4">
                    <CardTitle className="flex items-center gap-4 text-xl arabic-text">
                      <div
                        className={`w-12 h-12 bg-gradient-to-br ${policy.color} rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}
                      >
                  ...
                  <CardContent
                    className={`relative ${hasContent ? "pb-6" : "pb-8"}`}
                  >
                    {text ? (
                      <div className="text-muted-foreground leading-relaxed arabic-text">
                        <ol
                          dir="rtl"
                          className="list-decimal list-inside space-y-3 text-right arabic-text"
                        >
                          {text
                            .split(/\n+/) // نقسم النص إلى أسطر
                            .map((line) => line.trim())
                            .filter((line) => line !== "")
                            .map((line, i) => {
                              // نحذف الرموز والنقط أو الأرقام المكررة في بداية السطر
                      ...
                      <div className="text-center py-4">
                        ...
                        <p className="text-muted-foreground italic arabic-text text-sm">
                          سيتم إضافة المحتوى قريبًا…
                        </p>
                        <p className="text-xs text-muted-foreground mt-1 arabic-text">
                          نعمل على تحديث هذا القسم
                        </p>
                      </div>
                    )}
                  </CardContent>
          ...
          <Card className="bg-gradient-to-br from-primary to-purple-600 text-white border-0 overflow-hidden relative">
            ...
            <CardContent className="p-12 text-center relative">
              ...
              <p className="text-xl arabic-text mb-8 text-white/90 max-w-2xl mx-auto leading-relaxed">
                إذا كان لديك أي أسئلة أو تحتاج لتوضيحات إضافية حول أي من
                سياساتنا، لا تتردد في التواصل معنا
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-2xl mx-auto mb-8">
                <Card className="bg-white/10 border-white/20 backdrop-blur-sm">
                  <CardContent className="p-6 text-center">
                    ...
                    <p className="text-sm text-white/80 mb-2 arabic-text">
                      البريد الإلكتروني
                    </p>
                ...
                <Card className="bg-white/10 border-white/20 backdrop-blur-sm">
                  <CardContent className="p-6 text-center">
                    ...
                    <p className="font-medium text-white arabic-text">
                      متاح للاستفسارات
                    </p>
                  </CardContent>
                </Card>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Button
                  asChild
                  className="!text-white border-white/30  hover:bg-white/90 font-bold"
                >
                  <Link
                    href="/contact"
            ...
            <Card className="bg-slate-50 border-slate-200/70 inline-block">
              <CardContent className="p-6">
                ...
                <p className="text-xs text-slate-600 arabic-text">
                  تحتفظ بصمة تصميم بحق تعديل هذه السياسات في أي وقت مع إشعار
                  مسبق للعملاء
                </p>
              </CardContent>
            </Card>
