...
export default function LoginPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage =
        err?.message || err?.error || "حدث خطأ أثناء تسجيل الدخول";
      ...
      if (err?.requiresEmailVerification) {
        toast({
          ...
          description:
            "يجب تفعيل حسابك أولاً. يرجى فحص بريدك الإلكتروني والضغط على رابط التفعيل.",
          variant: "destructive",
        });
      } else {
        toast({
              ...
              <CardContent className="p-8 space-y-6">
                ...
                <div className="space-y-4 text-center">
                  <Link
                    ...
                    className="text-sm transition-colors arabic-text inline-block hover:underline "
                    style={{ color: "#000 !important", textDecoration: "none" }}
                  >
                    <span style={{ color: "#d1d5dc" }}>نسيت كلمة المرور؟</span>
                  </Link>
                  ...
                  <p className="text-gray-300 text-sm arabic-text">
                    ...
                    >
                      <span style={{ color: "#d1d5dc" }}>إنشاء حساب جديد</span>
                    </Link>
                  </p>
                </div>
              </CardContent>
